...
from userbot.utils import edit_or_reply, edit_delete, register
...
async def cmd_list(event):
    ...
    if args:
        if args in CMD_HELP:
            await edit_or_reply(event, f"**♨️ Commands available in {args} ♨️** \n\n" + str(CMD_HELP[args]) + "\n\n**☞ @KyuraProject**")
        else:
            await edit_delete(event, f"**Module** `{args}` **Tidak tersedia!**")
    else:
        try:
            results = await bot.inline_query(  # pylint:disable=E0602
                BOT_USERNAME, "@BAGASKARASUPPORTBOT"
            )
